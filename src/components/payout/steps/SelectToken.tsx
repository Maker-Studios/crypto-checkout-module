...
import { SelectNetworkSVG } from '@/components/SVGs'
...
import useChainStables from '@/hooks/useChainStables'
...
interface SelectNetworkProps {
  ...
  handleCoinSelect: (data: TokenData) => void
  handleStepChange: (step: CHECKOUTSTEPS) => void
}

const SelectNetwork = ({ handleStepChange, pricing, logo, selectedToken, handleCoinSelect }: SelectNetworkProps) => {
  const account = useAccount()
  const chainId = useChainId()
  const stableTokens = useChainStables()

  const disableNextStep =
    !account || account.status !== 'connected' || !selectedToken || selectedToken.chainId !== chainId

  console.log({ account, selectedToken })
  ...
  return (
    <motion.div className="flex flex-col space-y-6">
      ...
      <div className="space-y-1.5 text-center">
        <h4 className="text-2xl font-bricolage font-semibold">Select Token</h4>
        <p className="font-geist text-[16px] text-[#474747]">Choose your network and see available tokens.</p>
      </div>

      <div className="p-4 space-y-4 rounded-[24px] border border-[#F1F1F1]">
        <div className="flex items-center justify-center">
          <WalletConnectButton />
        </div>
        ...
        <div className="flex flex-col space-y-6">
          ...
          >
            ...
            <div>
              ...
              <AnimatePresence>
                {selectedToken ? (
                  <motion.p className="text-[13px] text-[#6F6F6F]">
            ...
            <span className="top-4 right-4 absolute z-10">{logo}</span>
            ...
            <svg
              ...
              className="absolute right-4 bottom-0 -z-0"
            >
              <mask
                id="mask0_863_2893"
          ...
          <div className="flex flex-col space-y-4">
            {/* <Select onValueChange={(value) => handleCoinSelect(value)}> */}
            <h2 className="font-geist text-[#737373] text-[13px] font-semibold">Select token</h2>
            <div className="flex flex-1 flex-col space-y-2">
              {account.address &&
                stableTokens.map((token) => (
                  <Suspense fallback={<span>Loading...</span>} key={token.name}>
                    <Token
                      {...token}
        ...
        <div className="flex items-center justify-center gap-4">
          <>
            ...
            <button
              className={cn(
                'h-10 px-4 py-3 bg-black text-white inline-flex items-center justify-center text-[13px] rounded-[32px] hover:opacity-85 transition-all duration-300 ease-in-out font-medium',
                disableNextStep && 'cursor-default pointer-events-none opacity-40'
              )}
              disabled={disableNextStep}
              onClick={() => {
                handleStepChange(CHECKOUTSTEPS.approve)
              }}
            >
              Continue
            </button>
