...
import { CHECKOUTSTEPS, TokenData } from '@/lib/types'
...
import ApprovalButton from '@/components/web3/ApprovalButton'
...
const ApproveTransaction = ({
  ...
  const handleApproveTransaction = async () => {
    ...
    try {
      const hash = await writeContractAsync({
        ...
        args: [checkoutContract, parseUnits(amount.toString(), selectedToken?.decimal)],
        // args: [checkoutContract, parseUnits(amount.toString(), selectedToken?.decimal)],
      })

      addRecentTransaction({
        hash,
        description: `Approve spend of ${amount} ${selectedToken.symbol}`,
      })
      ...
      console.log(`Transaction/Approval error occurred`)
    }
    setLoading(false)
  }
    ...
    try {
      const hash = await writeContractAsync({
        ...
        args: [
          ...
          parseUnits(amount.toString(), selectedToken.decimal),
          checkoutUID ?? '',
          false,
        ],
      })
      ...
      addRecentTransaction({
        hash,
        description: `Checkout with ${amount} ${selectedToken.symbol}`,
      })

      handleStepChange(CHECKOUTSTEPS['transaction-pending'])
    } catch (error) {
      console.log(`Transaction/Approval error occurred`)
    }
    setLoading(false)
  ...
  const requiresApproval =
    currentApproval.data === undefined
      ? true
      : amount > Number(formatUnits(currentApproval.data, selectedToken.decimal))
    ...
    <motion.div>
      ...
      {requiresApproval ? (
        <div className="mt-6 space-y-1.5 text-center">
          <h4 className="text-2xl font-bricolage font-semibold">Approve {selectedToken.symbol}</h4>
      ...
      <div className="mt-4 mb-8 space-y-4">
        <div className="flex items-center justify-center">
          <WalletConnectButton canChange={false} />
        </div>
        <div
        ...
        >
          ...
          <div>
            <p className="text-[#C4C4C4] text-[24px]">
              <Price price={amount} />
            </p>

            <p className="text-[13px] text-[#6F6F6F]">
              <Price price={amount} shouldFormat={false} /> {selectedToken?.symbol}
            </p>
          </div>
        ...
        {/* Add animation here for when this shows */}
        {/* TODO : use copy when not link | use visit when link */}
        {requiresApproval && approvalHash && (
            ...
            <p>
              ...
              {client?.chain.blockExplorers?.default.url ? (
                <Link
                  ...
                  target="_blank"
                  href={`${client?.chain.blockExplorers?.default.url}/tx/${approvalHash}`}
                >
                  <span className="text-[#0094FF]">
                    {approvalHash.slice(0, 6)}...{approvalHash.slice(-5)}
                  </span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
      ...
      <div className="flex items-center justify-center gap-4">
        <Suspense fallback={<span>Checking approval...</span>}>
          <>
            <button
              className={cn(
                'h-10 px-4 py-3 bg-[#F3F3F3] text-black inline-flex items-center justify-center text-[13px] rounded-[32px] hover:opacity-85 transition-all duration-300 ease-in-out font-medium',
                loading && 'pointer-events-none opacity-40'
              )}
              onClick={() => !loading && handleStepChange(CHECKOUTSTEPS['select-token'])}
            >
              Back
            </button>
            <ApprovalButton
              ...
              approvalCheckCompleted={currentApproval.data !== undefined}
            />
          </>
        </Suspense>
      </div>
